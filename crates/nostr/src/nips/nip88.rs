...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingOptionId => f.write_str("Missing option ID"),
            Self::MissingOptionText => f.write_str("Missing option text"),
            Self::MissingRelayUrl => f.write_str("Missing relay URL"),
            Self::MissingResponse => f.write_str("Missing response"),
...
impl TagCodec for Nip88Tag {
        ...
        S: AsRef<str>,
    {
        ...
        match kind.as_ref() {
            POLL_OPTION => {
                let id: S = iter.next().ok_or(Error::MissingOptionId)?;
                let text: S = iter.next().ok_or(Error::MissingOptionText)?;

                Ok(Self::PollOption(PollOption {
                    id: id.as_ref().to_string(),
                    text: text.as_ref().to_string(),
                }))
            }
            POLL_RESPONSE => {
                let response: S = iter.next().ok_or(Error::MissingResponse)?;
                Ok(Self::PollResponse(response.as_ref().to_string()))
            }
            POLL_TYPE => {
                let poll_type: S = iter.next().ok_or(Error::UnknownPollType)?;
                Ok(Self::PollType(PollType::from_str(poll_type.as_ref())?))
            }
            RELAY => {
            ...
            ENDS_AT => {
                ...
                Ok(Self::PollEndsAt(Timestamp::from_str(timestamp.as_ref())?))
            }
    ...
    pub fn from_event(event: &Event) -> Result<Self, Error> {
        ...
        for tag in event.tags.iter() {
            match Nip88Tag::try_from(tag) {
                ...
                Ok(Nip88Tag::PollResponse(..)) | Err(Error::UnknownTag) => (),
