...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingTagValue => f.write_str("Missing tag value"),
            Self::MissingTimestamp => f.write_str("Missing timestamp"),
            Self::MissingOffset => f.write_str("Missing offset"),
            Self::InvalidCommitId => f.write_str("Invalid commit ID"),
...
impl TagCodec for Nip34Tag {
        ...
        S: AsRef<str>,
    {
        ...
        match kind {
            ...
            BRANCH_NAME => {
                let name: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::BranchName(name.as_ref().to_string()))
            }
            CLONE => Ok(Self::Clone(parse_url_list(iter)?)),
            COMMIT => {
                let commit: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::Commit(parse_commit_id(commit.as_ref())?))
            }
            COMMIT_PGP_SIG => {
                let signature: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::CommitPgpSig(signature.as_ref().to_string()))
            }
            COMMITTER => {
                ...
                let offset_minutes: S = iter.next().ok_or(Error::MissingOffset)?;

                Ok(Self::Committer {
                    name,
                    email,
                    timestamp: Timestamp::from_str(timestamp.as_ref())?,
                    offset_minutes: i32::from_str(offset_minutes.as_ref())?,
                })
            }
            CURRENT_COMMIT => {
                let commit: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::CurrentCommit(parse_commit_id(commit.as_ref())?))
            }
            DESCRIPTION => {
                let description: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::Description(description.as_ref().to_string()))
            }
            GRASP => {
                let relay: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::Grasp(RelayUrl::parse(relay.as_ref())?))
            }
            HEAD => {
                let head: S = iter.next().ok_or(Error::MissingTagValue)?;
                let branch: &str = head
                    .as_ref()
            ...
            MERGE_BASE => {
                let commit: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::MergeBase(parse_commit_id(commit.as_ref())?))
            }
            MERGE_COMMIT => {
                let commit: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::MergeCommit(parse_commit_id(commit.as_ref())?))
            }
            NAME => {
                let name: S = iter.next().ok_or(Error::MissingTagValue)?;
            ...
            PARENT_COMMIT => {
                let commit: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::ParentCommit(parse_commit_id(commit.as_ref())?))
            }
            REFERENCE => {
                let commit: S = iter.next().ok_or(Error::MissingTagValue)?;
                let commit: Sha1Hash = parse_commit_id(commit.as_ref())?;

                match iter.next() {
                    Some(marker) if marker.as_ref() == EUC => {
                        Ok(Self::EarliestUniqueCommitId(commit))
                    }
                    Some(_) => Err(Error::UnknownTag),
                    None => Ok(Self::Reference(commit)),
                }
            }
            SUBJECT => {
                let subject: S = iter.next().ok_or(Error::MissingTagValue)?;
                Ok(Self::Subject(subject.as_ref().to_string()))
            }
            ...
            _ if kind.starts_with(REFS_HEADS) => {
                ...
                Ok(Self::RefHead {
                    ...
                    commit: parse_commit_id(commit.as_ref())?,
                })
            }
...
#[inline]
fn parse_commit_id(value: &str) -> Result<Sha1Hash, Error> {
    Sha1Hash::from_str(value).map_err(|_| Error::InvalidCommitId)
}
    ...
    S: AsRef<str>,
{
    let values: Vec<Sha1Hash> = iter
        .into_iter()
        .map(|value| parse_commit_id(value.as_ref()))
        .collect::<Result<_, _>>()?;

    if values.is_empty() {
        return Err(Error::MissingTagValue);
    }
