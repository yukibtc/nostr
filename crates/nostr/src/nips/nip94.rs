...
pub enum Error {
    ...
    /// Missing magnet URI
    MissingMagnet,
    /// Missing torrent infohash
    MissingTorrentInfohash,
    /// Missing blurhash
    MissingBlurhash,
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingSha256 => f.write_str("Missing SHA256"),
            Self::MissingOriginalHash => f.write_str("Missing original SHA256"),
            Self::MissingSize => f.write_str("Missing size"),
            Self::MissingDimensions => f.write_str("Missing dimensions"),
            Self::MissingMagnet => f.write_str("Missing magnet URI"),
            Self::MissingTorrentInfohash => f.write_str("Missing torrent infohash"),
            Self::MissingBlurhash => f.write_str("Missing blurhash"),
            Self::MissingThumbUrl => f.write_str("Missing thumbnail URL"),
            Self::MissingImageUrl => f.write_str("Missing image URL"),
            Self::MissingSummary => f.write_str("Missing summary"),
            Self::MissingAlt => f.write_str("Missing alt text"),
            Self::MissingFallback => f.write_str("Missing fallback URL"),
            Self::MissingService => f.write_str("Missing service"),
...
impl TagCodec for Nip94Tag {
        ...
        S: AsRef<str>,
    {
        ...
        match kind.as_ref() {
            ...
            MIME_TYPE => Ok(Self::MimeType(parse_mime_type_tag(iter)?)),
            SHA256 => Ok(Self::Sha256(parse_sha256_tag(iter)?)),
            ORIGINAL_HASH => Ok(Self::OriginalHash(parse_original_hash_tag(iter)?)),
            SIZE => Ok(Self::Size(parse_size_tag(iter)?)),
            DIMENSIONS => Ok(Self::Dim(parse_dim_tag(iter)?)),
            MAGNET => Ok(Self::Magnet(parse_magnet_tag(iter)?)),
            TORRENT_INFOHASH => Ok(Self::TorrentInfohash(parse_torrent_infohash_tag(iter)?)),
            BLURHASH => Ok(Self::Blurhash(parse_blurhash_tag(iter)?)),
            THUMB => {
                let (url, hash) = parse_thumb_or_image_tag(iter, Error::MissingThumbUrl)?;
                Ok(Self::Thumb { url, hash })
            }
            IMAGE => {
                let (url, hash) = parse_thumb_or_image_tag(iter, Error::MissingImageUrl)?;
                Ok(Self::Image { url, hash })
            }
            SUMMARY => Ok(Self::Summary(parse_summary_tag(iter)?)),
            ALT => Ok(Self::Alt(parse_alt_tag(iter)?)),
            FALLBACK => Ok(Self::Fallback(parse_fallback_tag(iter)?)),
            SERVICE => Ok(Self::Service(parse_service_tag(iter)?)),
...
fn parse_url_tag<T, S>(mut iter: T) -> Result<Url, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
...
fn parse_mime_type_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let mime_type: S = iter.next().ok_or(Error::MissingMimeType)?;
    Ok(mime_type.as_ref().to_string())
}

fn parse_sha256_tag<T, S>(mut iter: T) -> Result<Sha256Hash, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let hash: S = iter.next().ok_or(Error::MissingSha256)?;
    Ok(Sha256Hash::from_str(hash.as_ref())?)
}

fn parse_original_hash_tag<T, S>(mut iter: T) -> Result<Sha256Hash, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let hash: S = iter.next().ok_or(Error::MissingOriginalHash)?;
    Ok(Sha256Hash::from_str(hash.as_ref())?)
}

fn parse_size_tag<T, S>(mut iter: T) -> Result<usize, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let size: S = iter.next().ok_or(Error::MissingSize)?;
    Ok(size.as_ref().parse()?)
}

fn parse_dim_tag<T, S>(mut iter: T) -> Result<ImageDimensions, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let dim: S = iter.next().ok_or(Error::MissingDimensions)?;
    Ok(ImageDimensions::from_str(dim.as_ref())?)
}

fn parse_magnet_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let magnet: S = iter.next().ok_or(Error::MissingMagnet)?;
    Ok(magnet.as_ref().to_string())
}

fn parse_torrent_infohash_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let infohash: S = iter.next().ok_or(Error::MissingTorrentInfohash)?;
    Ok(infohash.as_ref().to_string())
}

fn parse_blurhash_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let blurhash: S = iter.next().ok_or(Error::MissingBlurhash)?;
    Ok(blurhash.as_ref().to_string())
}
...
where
    ...
    S: AsRef<str>,
{
    let url: S = iter.next().ok_or(missing_url)?;
...
fn parse_summary_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let summary: S = iter.next().ok_or(Error::MissingSummary)?;
    Ok(summary.as_ref().to_string())
}

fn parse_alt_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let alt: S = iter.next().ok_or(Error::MissingAlt)?;
    Ok(alt.as_ref().to_string())
}

fn parse_fallback_tag<T, S>(mut iter: T) -> Result<Url, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let url: S = iter.next().ok_or(Error::MissingFallback)?;
    Ok(Url::parse(url.as_ref())?)
}

fn parse_service_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let service: S = iter.next().ok_or(Error::MissingService)?;
    Ok(service.as_ref().to_string())
}
