...
impl TagCodec for Nip53Tag {
        ...
        S: AsRef<str>,
    {
        ...
        match kind.as_ref() {
            TITLE => Ok(Self::Title(parse_string_tag(iter)?)),
            SUMMARY => Ok(Self::Summary(parse_string_tag(iter)?)),
            IMAGE => {
                let image: S = iter.next().ok_or(Error::UnknownTag)?;
                let image: Url = Url::parse(image.as_ref())?;
                let dimensions = match iter.next() {
                    Some(value) => Some(ImageDimensions::from_str(value.as_ref())?),
                    None => None,
                };
                Ok(Self::Image(image, dimensions))
            }
            "t" => {
                let hashtag: String = parse_string_tag(iter)?;
            ...
            STREAMING => Ok(Self::Streaming(parse_url_tag(iter)?)),
            RECORDING => Ok(Self::Recording(parse_url_tag(iter)?)),
            STARTS => Ok(Self::Starts(parse_timestamp_tag(iter)?)),
            ENDS => Ok(Self::Ends(parse_timestamp_tag(iter)?)),
            STATUS => Ok(Self::Status(LiveEventStatus::from(parse_string_tag(iter)?))),
            CURRENT_PARTICIPANTS => Ok(Self::CurrentParticipants(parse_u64_tag(iter)?)),
            TOTAL_PARTICIPANTS => Ok(Self::TotalParticipants(parse_u64_tag(iter)?)),
            RELAYS => {
            ...
            "p" => parse_p_tag(iter),
            ROOM => Ok(Self::Room(parse_string_tag(iter)?)),
            SERVICE => Ok(Self::Service(parse_url_tag(iter)?)),
            ENDPOINT => Ok(Self::Endpoint(parse_url_tag(iter)?)),
            PINNED => {
                let event_id: S = iter.next().ok_or(Error::MissingEventId)?;
                Ok(Self::Pinned(EventId::from_hex(event_id.as_ref())?))
            }
            "a" => parse_a_tag(iter),
...
fn parse_string_tag<T, S>(mut iter: T) -> Result<String, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let value: S = iter.next().ok_or(Error::UnknownTag)?;
...
{
    let value: S = iter.next().ok_or(Error::UnknownTag)?;
    Ok(Url::parse(value.as_ref())?)
}

fn parse_timestamp_tag<T, S>(mut iter: T) -> Result<Timestamp, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    let value: S = iter.next().ok_or(Error::UnknownTag)?;
    Ok(Timestamp::from_str(value.as_ref())?)
}

fn parse_u64_tag<T, S>(mut iter: T) -> Result<u64, Error>
where
    T: Iterator<Item = S>,
    S: AsRef<str>,
{
    ...
    S: AsRef<str>,
{
    ...
    let relay_url = match iter.next() {
        Some(value) if value.as_ref().is_empty() => None,
        Some(value) => Some(RelayUrl::parse(value.as_ref())?),
        None => return Err(Error::UnknownTag),
    };

    let marker: S = iter.next().ok_or(Error::UnknownTag)?;
    let marker = LiveEventMarker::from_str(marker.as_ref())?;
    let proof = iter
        .next()
        .and_then(|value| Signature::from_str(value.as_ref()).ok());
    ...
    S: AsRef<str>,
{
    ...
    let coordinate = Coordinate::parse(coordinate.as_ref())?;
    let relay_url = match iter.next() {
        Some(value) if value.as_ref().is_empty() => None,
        Some(value) => Some(RelayUrl::parse(value.as_ref())?),
        None => None,
    };
    let marker = iter.next().map(|value| value.as_ref().to_string());
...
where
    ...
    S: AsRef<str>,
{
    let hand: S = iter.next().ok_or(Error::UnknownTag)?;
    let hand = match hand.as_ref() {
        "1" => true,
        "0" => false,
