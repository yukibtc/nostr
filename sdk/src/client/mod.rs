...
impl Client {
    ...
    /// conditions are met (if configured with `.close_on()`).
    ///
    /// **Important**: This method creates a new subscription. None of the previous subscriptions
    /// will be edited/closed when you call this! Remember to unsubscribe when you no longer need it.
    /// You can get all your active (non-auto-closing) subscriptions by calling `client.subscriptions().await`.
    ///
    /// ## Gossip Support
    ///
    ...
    ///
    /// It's possible to automatically close a subscription by configuring the [`SubscribeAutoCloseOptions`] (see below examples).
    ///
    ...
    // TODO: deprecate this? Now the send_event allow choosing where to send an event
    /// Send a private direct message to the NIP-17 relays of the receiver.
    ///
    ///
    /// This method requires that [`NostrSigner`] and [`NostrGossip`] are configured for the [`Client`].
    ///
    /// # Errors
    ///
        ...
        I: IntoIterator<Item = Tag>,
    {
        ...
        self.send_event(&event).to_nip17().await
    }
        ...
        IT: IntoIterator<Item = Tag>,
    {
        ...
        // Send
        self.send_event(&gift_wrap).to(urls).await
    }
...
impl Client {
    ...
    async fn check_and_update_gossip_fetch(
        ...
        for chunk in filters.chunks(10) {
            ...
            let mut stream = self
                .pool
                .stream_events(targets, Duration::from_secs(10), ReqExitPolicy::ExitOnEOSE)
                .await?;
